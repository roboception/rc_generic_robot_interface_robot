...
data = Array_Append(data, GetBytes(11, 0)) // action
...
while (True){  // send to the socket until there is a response
    ...
    if (Length(response) == 0) {
        ...
        if (error_code == 0)
        {
            ...
            Display("Camera set to static mounting", success_string)
        } else { 
            // on failure
            ...
            Display("Black", "Red", "Camera set to static mounting error", error_string)
        }
        break
    }
}
