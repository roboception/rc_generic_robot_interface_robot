...
for (int slot = 1; slot < 9; slot++) {
    ...
    data = Array_Append(data, GetBytes(17, 0)) // action
    ...
    data = Array_Append(data, GetBytes(0, 0, 0)) // w - only used for quaternions
    data = Array_Append(data, GetBytes(slot, 0, 0)) // data 1
    ...
    data[2] = Length(data) // Update the data length
    ...
    // Unpack the response
    if (Length(response) == 0) {
        Display("No response received for slot " + slot)
    } else {
        ...
        if (error_code == 0) {
            // on success
            string success_string = "For slot " + slot
            Display("Pose set successfully", success_string)
        } else { 
            // on failure
            ...
            Display("Black", "Red", "Set pose error", error_string)
        }
